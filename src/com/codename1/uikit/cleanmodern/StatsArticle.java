...
public class StatsArticle extends BaseForm {
    ...
    Form current;
BaseForm form;
        public StatsArticle(Resources res)  {
...
*/
    ...
    Map<String, Integer> articleStats = serviceArticle.getEventStatsByType();
    ...
    int index = 0;
    for (Map.Entry<String, Integer> entry : articleStats.entrySet()) {
    ...
    // Create the chart
    PieChart chart = new PieChart(buildDataset("Article , auteur ", articleStats), renderer);

    // Add the chart to a component
    ChartComponent c = new ChartComponent(chart);

    String[] messages = {"Article stats by author"};
    SpanLabel message = new SpanLabel(messages[0], "WelcomeMessage");
    ...
    private CategorySeries buildDataset(String title, Map<String, Integer> articleStats) {
    ...
    private CategorySeries buildDataset(String title, double prcntFeed, double prcntRec) {
        ...
        series.add("Article",prcntRec);
        series.add("Autheur",prcntFeed);
